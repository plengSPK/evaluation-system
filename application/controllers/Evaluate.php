...
class Evaluate extends CI_Controller {
	...
	{		
        ...
        $score_teamwork = $this->input->post('score_creativity');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            // $val_check = $this->evaluate_model->checkAlreadyRecorded($user_detail['user_id'], $target_id, $curQuarter, $curYear);
            // echo $val_check;
    ...
    public function detail($evalaute_id = 0)
	{		
        if($evalaute_id == 0){
            $this->index();
        }
        ...
        $user_detail = $this->session->user_detail;
        $data['user_detail'] = $user_detail;
        $data_detail['evalaute_id'] = $evalaute_id;
    ...
    public function result($evalaute_id = 0)
	{		
        if($evalaute_id == 0){
            $this->index();
        }
